...
class FormGroup extends FormPart {
		...
		if(!is_a($parent, 'FormGroup'))
			return $this->get_specific_html($parent, $defaults);
                ...
                $tag = tag('tr', attributes("class=\"{$this->class}\""));
		$cell = tag('td');

                if($this->title !== false)
			$tag->add(tag('th', $this->title));
                else
			$cell->add(attributes("colspan=\"2\""));
		...
		$cell = tag('td');
			...
			$cell->add(attributes("colspan=\"2\""));
                ...
                if($this->description !== false)
                        $cell->add(tag('p', attributes('class="form-question-description"'),
						$this->description));
...
class FormLongFreeQuestion extends FormAtomicQuestion {
	...
	var $cols;
		...
		$tag = tag('textarea', attributes("rows=\"{$this->rows}\"",
					"name=\"{$this->qid}\"",
					"cols=\"{$this->cols}\""), $text);
		return tag('div', attributes("class=\"form-textarea\""), $tag);
        }
}
...
class FormSequenceQuestion extends FormAtomicQuestion {
	...
	var $name_func;
		...
		$input = create_select_range($this->qid, $this->lbound,
				...
				$this->name_func);
                return tag('div', attributes("class=\"{$this->class}\""), $day);
        }
}

/* creates a hidden input
 * FIXME: make this handle a default. this may create unwanted rows
 */
	...
	function get_specific_html($parent, $defaults = array()) {
		...
		foreach($this->options as $key => $name) {
			...
			if(!empty($this->conditionals[$key])) {
				...
				$tag->add(tag('table', attributes("id=\"{$this->qid}-{$key}\""),
...
class FormDropdownQuestion extends FormCompoundQuestion {
        ...
        function get_specific_html($parent, $defaults = array()) {
		$select = tag('select', attributes("id=\"{$this->qid}\"",
					"name=\"{$this->qid}\"",
					'class="form-select"'));
...
class Form extends FormGroup {
        ...
        function get_html($defaults = array()) {
                $form_attrs = attributes("action=\"{$this->action}\"",
				'method="POST"');
                if($this->method !== false) {
                        $form_attrs->add("method=\"{$this->method}\"");
                }
		$table = tag('table', attributes("class=\"{$this->class}\""));
                foreach($this->list as $child) {
                        $table->add($child->get_html($this, $defaults));
                }
		return tag('form', $form_attrs,
				tag("script",
