...
function import() 
{
    ...
    $old_dbh = @new mysqli(
        $vars['host'], $vars['username'], $vars['passwd'],
        $vars['dbname'], $port
    );
    ...
    or $phpcdb->db_error(
        __('Error selecting events in import'),
        $query
    );
    ...
    while($result = $sth->fetch_assoc()) {
        ...
        if(empty($username) || strlen($username) == 0) {
            ...
            if(!isset($users[$username])) {
                $users[$username] = $phpcdb->create_user(
                    $username,
        ...
        $final_ts = mktime(
            $hour, $minute, 0, $result['endmonth'],
            $result['endday'], $result['endyear']
        );

        while(true) {
            $start_ts = mktime(
                $hour, $minute, 0, $month, $day,
                $year
            );
    ...
    return message_redirect(
        sprintf(
            __("Created %s events with %s occurences"),
            $events, $occurrences
        ),
        $form_page
    );
