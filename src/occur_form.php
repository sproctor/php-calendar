...
function display_form() 
{
    ...
    if(isset($vars['oid'])) {
        ...
        $start_date = format_short_date_string(
            $occ->get_start_year(),
            $occ->get_start_month(), $occ->get_start_day(),
            $datefmt
        );
        $end_date = format_short_date_string(
            $occ->get_end_year(),
...
function process_form()
{
    ...
    if(!isset($vars['oid'])) {
        ...
        $oid = $phpcdb->create_occurrence(
            $vars["eid"], $time_type, $start_ts,
            $end_ts
        );
    } else {
        $modify = true;
        $oid = $vars["oid"];
        $phpcdb->modify_occurrence(
            $oid, $time_type, $start_ts,
            $end_ts
        );
    }
    ...
    if($oid != 0) {
        ...
        return message_redirect(
            __('Error submitting occurrence.'),
