...
use PhpCalendar\DisplayFunctions;

class MonthController
{

// Full display for a month
public function showAction($twig, $context, $cid, $year, $month)
{
	global $home_url, $templates_path, $comp_cache;

	$months = array();
	for($i = 1; $i <= 12; $i++) {
		$m = month_name($i);
		$months["$home_url?action=display_month&amp;phpcid=$cid&amp;month=$i&amp;year=$year"] = $m;
	}
	$years = array();
	for($i = $year - 5; $i <= $year + 5; $i++) {
		$years["$home_url?action=display_month&amp;phpcid=$cid&amp;month=$month&amp;year=$i"] = $i;
	}
	$next_month = $month + 1;
	$next_year = $year;
	if($next_month > 12) {
	...
	$heading = tag('',
			tag('a', attrs('class="phpc-icon-link"',
					"href=\"$home_url?action=display_month&amp;phpcid=$cid&amp;month=$prev_month&amp;year=$prev_year\""),
				tag('span', attrs('class="fa fa-chevron-left"'), '')),
			create_dropdown_list(month_name($month), $months),
			create_dropdown_list($year, $years),
			tag('a', attrs('class="phpc-icon-link"',
					"href=\"$home_url?action=display_month&amp;phpcid=$cid&amp;month=$next_month&amp;year=$next_year\""),
				tag('span', attrs('class="fa fa-chevron-right"'), '')));

	return new Response($twig->render('month.html.twig', $context));
	return new Response(DisplayFunctions::create_display_table($heading, MonthController::create_month($month,
				$year))->toString());
}
...
function create_month($month, $year)
{
	...
	$days_events = DisplayFunctions::get_events($from_stamp, $to_stamp);
	$week_list = array();
	for($week_of_month = 1; $week_of_month <= $weeks; $week_of_month++) {
		...
		$week_list[] = DisplayFunctions::create_week($start_stamp, $year, $days_events);
	}
