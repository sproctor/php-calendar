...
class Tag
{
	...
	function add()
	{
		...
		foreach ($htmlElements as $htmlElement) {
			...
			} elseif (is_object($htmlElement) && !$htmlElement instanceof Tag) {
...
function create_select($name, $options, $default = null, $attrs = null)
{
	...
	foreach ($options as $value => $text) {
		...
		$select->add(new Tag('option', $attributes, $text));
	}
...
function create_multi_select($name, $option_lists, $default = null, $attrs = null)
{
	...
	foreach ($option_lists as $category => $options) {
		if (is_array($options)) {
			$group = new Tag('optgroup', new AttributeList("label=\"$category\""));
			$select->add($group);
			foreach ($options as $value => $text) {
				$attributes = new AttributeList("value=\"$value\"");
				if ($value === $default)
				...
				$group->add(new Tag('option', $attributes, $text));
			}
		} else {
			$value = $options;
			$text = $category;
