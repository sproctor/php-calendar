...
function calendar_list()
{
    ...
    foreach($phpcdb->get_calendars() as $calendar) {
        ...
        if ($cid == $default_cid) {
            ...
            $default_tag = create_action_link(
                __('Make Default'),
                "default_calendar",
                array('cid' => $cid),
    ...
    $dialog = tag(
        'div', attrs('id="phpc-dialog-cal-del"', 'title="' . __("Confirmation required") . '"'),
        __("Permanently delete this calendar?")
    );

        return tag(
            'div', attrs('id="phpc-admin-calendars"'),
            $dialog,
            tag(
...
function user_list()
{
    ...
    foreach($phpcdb->get_users() as $user) {
        ...
        if (!$user->is_disabled()) {
            $disable_link =  create_action_link(
                __("Disable"),
                "user_disable",
                array("uid" => $user->uid)
            );
...
function import() 
{
    ...
    return tag(
        'div', attrs('id="phpc-admin-import"'),
